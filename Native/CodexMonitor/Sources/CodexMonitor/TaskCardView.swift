...
struct TaskCardView: View {
      ...
      Button(action: onSelect) {
        VStack(alignment: .leading, spacing: 10) {
          HStack(alignment: .top, spacing: 10) {
            ...
            Spacer(minLength: needsAttentionBadge ? 40 : 0)
          }
          ...
          HStack(spacing: 8) {
            ...
            Text(
              task.ownership == .hostedLive
                ? "工作台管理" : task.isExternallyObserved ? "本机观察" : "历史档案")
      ...
      RoundedRectangle(cornerRadius: 9)
        .stroke(
          cardFocused
            ? CMColor.focusRing
            : selected
              ? CMColor.ink : isCardHovered ? CMColor.ink.opacity(0.48) : CMColor.hairline,
          lineWidth: cardFocused
  ...
  private var needsAttentionBadge: Bool {
    task.displayStatus == .needsApproval || task.displayStatus == .needsInput
