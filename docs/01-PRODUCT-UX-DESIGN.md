...
“交办任务”使用主窗口 sheet，不增加一级界面；“全部会话”默认混排托管职员、本机观察会话与历史档案，并通过清晰的“工作台管理 / 本机观察 / 历史档案”标记区分；“历史档案”可用于单独筛选。

卡片单击用于选中并打开工作单；卡片右下角外跳按钮和工作单中的“在 Codex 中打开”统一调用官方 `codex://threads/<thread-id>` 深链接。无合法 UUID、无链接处理客户端或打开失败时留在工作台并显示可操作提示。
...
- `⌘K` 聚焦搜索。
- `⌘1…⌘5` 切换主要状态筛选。
- `Tab` / `Shift-Tab` 在项目、筛选、卡片和操作按钮之间移动焦点。
- `Return` / `Space` 执行当前聚焦控件；卡片打开工作单，“在 Codex 中打开”保持为独立操作。
